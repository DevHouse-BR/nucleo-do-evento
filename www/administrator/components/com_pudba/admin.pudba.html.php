...
class clspuminiadmin
{
	...
	{
		global $dbh,$DB,$sqldr,$reccount,$is_sht,$livesite,$self,$database,$pagelinks;
		$database->setQuery($q);
		$database->query();
		...
		$sqldr.="<table border='0' cellpadding='1' cellspacing='1' width=100% class='adminlist'>";
		$headers="<tr>";
		if ($is_select)
			$headers .= "<th><center>".PUD_EDIT."</center></th><th><center>".PUD_DELETE."</center></th>";
		//if ($is_sht) 
		//$headers.="<th><input type='checkbox' name='cball' value='' onclick='chkall(this)'></th>";
		for($i=0;$i<$fields_num;$i++)
		{
 		...
 		if ($is_sht) 
			$headers.="<th>".PUD_INSERT."</th><th>".PUD_EXPLAIN."</th><th>".PUD_INDEXES."</th><th>".PUD_EXPORT."</th><th>".PUD_DROP."</th><th>".PUD_TRUNCATE."</th><th>".PUD_OPTIMIZE."</th>";
 		$headers.="</tr>\n";
 		$sqldr.=$headers;
 		$swapper=false;
		$k = 0;
 		while($row=mysql_fetch_row($sth))
		{
   			...
   			for($i=0;$i<$fields_num;$i++)
			{
      				...
      				if ($is_sht && $i==0 && $v)
				{
         				...
         				."<td><center><a href=\"?option=com_pudba&db=$dbn&q=optimize+table+$v\" onclick='return ays()'><img src=$livesite/administrator/images/day_f2.png border=0 height=16 width=16></a>";
      				}
      				if ($is_show_crt) 
					$v="<pre>$v</pre>";
      				else
	...
	{
 		global $out_message, $SQLq, $err_msg, $reccount, $time_all, $sqldr, $page, $MAX_ROWS_PER_PAGE;
		?>
		<center>
	...
	function WritePagination($pageno,$url,$count)
	{
		global $RowsPerPage;
		if ($count > 0)
			$lastpage = ceil($count/$RowsPerPage);
		else
			$lastpage = 1;
		$pageno = (int)$pageno;
		if ($pageno < 1) 
