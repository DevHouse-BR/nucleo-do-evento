...
echo "<div class=tb>".$sqldr."</div><br>".$pagelinks;
if ($reccount > 0)
...
function do_sql($q,$tablename=""){
 global $dbh,$last_sth,$last_sql,$reccount,$out_message,$SQLq,$pageno;
 $SQLq=$q;
 ...
 if (!do_multi_sql($q,'',1)){
    ...
    if ($last_sth && $last_sql){
       ...
       if (preg_match("/^select|show|explain/i",$last_sql)) {
          ...
          clspuminiadmin::display_select($last_sth,$last_sql,$pageno);
       } else {
...
{
	global $last_sth,$last_sql,$limitstart,$limit,$pageno,$RowsPerPage;
 	$sql=trim($sql);
