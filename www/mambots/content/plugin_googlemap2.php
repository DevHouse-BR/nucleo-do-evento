				...
				if (tst".$id."_".$mapnm."_".$counter.".offsetWidth > 0) {
					...
					if (($inline_coords == 1)||($inline_coords == 0 && empty($kml))) {
						...
						if ((trim($text)!='')||$dir==1) {
							...
							if ($dir=='1') {
									$text.="<form action='http://maps.google.com/maps' method='get' target='_blank' onsubmit='DirectionMarkersubmit".$id."_".$mapnm."_".$counter."(this);return false;'>";
							    $text.="<br />".$txt_dir."<input type='radio' checked name='dir' value='to'> <b>".$txt_to."</b> <input type='radio' name='dir' value='from'><b>".$txt_from."</b>";
							    $text.="<br />".$txt_diraddr."<input type='text' class='inputbox' size='20' name='saddr' id='saddr' value='' /><br />";
							    $text.="<input value='".$txt_get_dir."' class='button' type='submit' style='margin-top: 2px;'>";
								
								if (!empty($address))
								    $text.="<input type='hidden' name='daddr' value='".$address." (".$latitude.", ".$longitude.")'/></form>";
								else
								    $text.="<input type='hidden' name='daddr' value='".$latitude.", ".$longitude."'/></form>";
							}
