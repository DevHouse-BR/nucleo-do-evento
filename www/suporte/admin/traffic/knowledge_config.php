...
<?
	...
	{
		LIST( $deptid, $parentid ) = explode( "<:>", $HTTP_POST_VARS['category'] ) ;
		Knowledge_put_Category( $dbh, $session_setup['aspID'], $deptid, $parentid, $HTTP_POST_VARS['name'], $HTTP_POST_VARS['order'] ) ;
	...
	{
		LIST( $deptid, $parentid ) = explode( "<:>", $HTTP_POST_VARS['category'] ) ;
		Knowledge_put_Question( $dbh, $session_setup['aspID'], $deptid, $parentid, $HTTP_POST_VARS['question'], $HTTP_POST_VARS['answer'] ) ;
		HEADER( "location: knowledge_config.php?action=config" ) ;
		exit ;
	}
	else if ( $action == "remove_quest" )
	{
		Knowledge_remove_Question( $dbh, $session_setup['aspID'], $HTTP_GET_VARS['questid'] ) ;
		HEADER( "location: knowledge_config.php?action=config" ) ;
		exit ;
	}
	else if ( $action == "remove_cat" )
	{
		Knowledge_remove_Category( $dbh, $session_setup['aspID'], $HTTP_GET_VARS['catid'] ) ;
		HEADER( "location: knowledge_config.php?action=config" ) ;
		exit ;
	}
	else if ( $action == "status" )
	{
		...
		if ( $total_questions || !$HTTP_GET_VARS['status'] )
		{
			AdminASP_update_TableValue( $dbh, $session_setup['aspID'], "knowledgebase", $HTTP_GET_VARS['status'] ) ;
			$HTTP_SESSION_VARS['session_setup']['knowledgebase'] = $HTTP_GET_VARS['status'] ;
	...
	{
		Knowledge_update_Category( $dbh, $session_setup['aspID'], $HTTP_POST_VARS['catid'], $HTTP_POST_VARS['name'], $HTTP_POST_VARS['order'] ) ;
		HEADER( "location: knowledge_config.php?action=config" ) ;
		exit ;
	}
	else if ( $action == "do_update_question" )
	{
		Knowledge_update_Question( $dbh, $session_setup['aspID'], $HTTP_POST_VARS['questid'], $HTTP_POST_VARS['question'], $HTTP_POST_VARS['answer'] ) ;
		HEADER( "location: knowledge_config.php?action=config" ) ;
		exit ;
	}
?>
...
<tr> 
  <td width="100%" valign="top">
	...
	<? else: ?>
		...
		Set your Knowledge Base Status (available to the public, etc.).  When you are building or updating your Knowledge Base, it may be helpful to turn it off to the public.
		</p>
	<? endif ; ?>
	...
	<tr class="altcolor2"">
		...
		<td valign="top"><span class="basetxt">
			<select name="category" size=13 style="width:<? echo $select_width ?>" OnClick=check_visible(this.value)>
	...
	<tr class="altcolor2"">
		...
		<td><input type="text" name="name" size="<? echo $text_width ?>" maxlength="255"></td>
	</tr>
	<tr class="altcolor2"">
	...
	<tr class="altcolor2"">
		...
		<td valign="top"><span class="basetxt">
			<select name="category" size=6 style="width:<? echo $select_width ?>" OnClick=check_visible(this.value)>
			<?
				for ( $c = 0; $c < count( $departments ); ++$c )
				{
					$department = $departments[$c] ;
	...
	<tr class="altcolor2"">
		...
		<td><input type="text" name="question" size="<? echo $text_width ?>" maxlength="255"></td>
	</tr>
	<tr class="altcolor2"">
		<td>Answer <font color="#FF0000">*</font><br>(HTML ok)</td>
		<td><textarea cols="<? echo $text_width ?>" rows=10 class="input" name="answer" wrap="virtual"></textarea></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
	...
	<? elseif ( $action == "config" ): ?>
		...
		<? if ( AdminUsers_get_TotalDepartments( $dbh, $session_setup['aspID'] ) > 0 ): ?>
		<table cellspacing=0 cellpadding=2 border=0>
		<tr>
			<td><a href="knowledge_config.php?action=create_cat"><img src="<? echo $BASE_URL ?>/images/knowledge/folder_closed.gif" border=0 alt=""></td>
			<td><a href="knowledge_config.php?action=create_cat">Create New Category</a></td>
		</tr>
		<tr>
			<td><a href="knowledge_config.php?action=create_question"><img src="<? echo $BASE_URL ?>/images/knowledge/document.gif" border=0 alt=""></td>
		...
		<? else: ?>
		<span class="hilight">Before you can build your Knowledge Base, you need to <a href="<? echo $BASE_URL ?>/setup/adddept.php">Create a Support Department</a>.</span>
		<? endif ; ?>
		If a visitor rates the Q&A as helpful, a point will be added to the <i>"help index"</i>.  But if the visitor rates the Q&A as not helpful, then a point is deducted from the <i>"help index"</i>.  Thus, if a question has a negative or very low <i>"help index"</i>, you may want to modify the Q&A.  The <i>"help index"</i> is also the Q&A display order.
		<p>
	<?
		$l = $x = 0 ;
		$departments = AdminUsers_get_AllDepartments( $dbh, $session_setup['aspID'], 1 ) ;
		...
		for ( $c = 0; $c < count( $departments ); ++$c )
		{
			...
			print "<table cellspacing=0 cellpadding=2 border=0><tr><td valign=\"top\"><img src=\"$BASE_URL/images/knowledge/folder_closed.gif\" border=0 alt=\"$name\"></td><td valign=\"top\"><span class=\"basetxt\"><b>$name $display_string</b><br>" ;
			...
			for ( $c2 = 0; $c2 < count( $deptcats ); ++$c2 )
			{
				...
				print "<table cellspacing=0 cellpadding=2 border=0><tr><td><img src=\"$BASE_URL/images/spacer.gif\" width=16 height=1></td><td valign=\"top\"><img src=\"$BASE_URL/images/knowledge/folder_closed.gif\" border=0 alt=\"$name\"></td><td valign=\"top\"><span class=\"basetxt\"><b>$name</b> [<a href=\"knowledge_config.php?action=edit_cat&catid=$category[catID]\">edit</a>] [<a href=\"JavaScript:remove_cat( $category[catID] )\">remove</a>]<br>" ;
	...
	<?
		...
		$catinfo = Knowledge_get_CatInfo( $dbh, $session_setup['aspID'], $HTTP_GET_VARS['catid'] ) ;
	?>
	<span class="basicTitle">Edit Category</span><br>

	<form method="POST" action="knowledge_config.php" name="form">
	<input type="hidden" name="action" value="do_update_cat">
	<input type="hidden" name="catid" value="<? echo $HTTP_GET_VARS['catid'] ?>">
	<table cellspacing=0 cellpadding=2 border=0 width="100%">
	<tr class="altcolor2"">
		<td>Category Name <font color="#FF0000">*</font></td>
		<td><input type="text" name="name" size="<? echo $text_width ?>" maxlength="255" value="<? echo preg_replace( "/\"/", "&quot;", stripslashes( $catinfo['name'] ) ) ?>"></td>
	</tr>
	<tr class="altcolor2"">
		<td>Display Order <font color="#FF0000">*</font></td>
		<td><input type="text" name="order" size=2 maxlength="3" onKeyPress="return numbersonly(event)" value="<? echo $catinfo['display_order'] ?>"> (numbers only)</td>
	</tr>
	<tr>
		<td>&nbsp;</td>
	...
	<?
		...
		$question = Knowledge_get_QuestInfo( $dbh, $session_setup['aspID'], $HTTP_GET_VARS['questid'] ) ;
	?>
	<span class="basicTitle">Edit Question and Answer</span><br>

	<form method="POST" action="knowledge_config.php" name="form">
	<input type="hidden" name="action" value="do_update_question">
	<input type="hidden" name="questid" value="<? echo $HTTP_GET_VARS['questid'] ?>">
	<table cellspacing=0 cellpadding=2 border=0 width="100%">
	<tr class="altcolor2"">
		<td>Question <font color="#FF0000">*</font></td>
		<td><input type="text" name="question" size="<? echo $text_width ?>" maxlength="255" value="<? echo preg_replace( "/\"/", "&quot;", stripslashes( $question['question'] ) ) ?>"></td>
	</tr>
	<tr class="altcolor2"">
		<td>Answer <font color="#FF0000">*</font><br>(HTML ok)</td>
		<td><textarea cols="55" rows=10 class="input" name="answer" wrap="virtual"><? echo stripslashes( $question['answer'] ) ?></textarea></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
	...
	?>
	<p>
	<span class="hilight"><? echo $error_mesg ?></span>
	...
	<input type="hidden" name="action" value="status">
	<tr>
		<td><input type="radio" value=1 name="status" <?= ( $session_setup['knowledgebase'] ) ? "checked" : "" ?>> Available to the Public <? echo $warning ?></td>
	</tr>
	<tr>
		<td><input type="radio" value=0 name="status" <?= ( !$session_setup['knowledgebase'] ) ? "checked" : "" ?>> Not Available to the Public (the link will not be shown)</td>
	</tr>
	<tr>
