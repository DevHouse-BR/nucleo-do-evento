...
<?
	...
	else if ( 0 )
	{
		...
		// For all license inquiries, please email audit@thesite.com
		//
		$level = ( isset( $HTTP_GET_VARS['level'] ) ) ? $HTTP_GET_VARS['level'] : 0 ;
		$url = $BASE_URL ;
		$os = $HTTP_SERVER_VARS['SERVER_SOFTWARE'] ;
		$os = urlencode( $os ) ;
...
--------------------------------------
" ;
			$greeting = "Welcome %%user%%! Please hold while we contact a representative. If a representative does not respond in a few seconds, then he/she is not available at this time.<hr>Live Support <hr>" ;
			$greeting = addslashes( $greeting ) ;
			$leave_msg = "Department is currently unavailable. Please leave a message.<hr>Live Support <hr>" ;
