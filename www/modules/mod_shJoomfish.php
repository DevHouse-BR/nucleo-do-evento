...
* Modified  by Yannick Gaultier, for better SEF URL generation handling
* 
* {shSourceVersionTag: V 1.2.4.m - 2007-05-04} 
*
**/
...
switch ( $type ) {
	...
	case 'dropdown':			// drop down of names
		if ( count($langActive)>0 ) {
			...
			{
				// shumisha fix for 404SEF 2007-04-07
        //$href = JFModuleHTML::_createHRef ($language->iso);
        $href = JFModuleHTML::_createHRef ($language->iso, $language->code);
        // shumisha end of fix
        ...
        $href = JFModuleHTML::_createHRef (null, null);
        // shumisha end of fix
...
if( $inc_jf_css && file_exists( $mosConfig_absolute_path. '/modules/mod_shJoomfish.css' ) ) {
	?>
<link href="<?php echo $mosConfig_live_site;?>/modules/mod_shJoomfish.css" rel="stylesheet" type="text/css"/>
	<?php
}
?>
<?php echo $outString;?>
...
class JFModuleHTML {
  ...
  function _createHRef( $iso, $shLangCode ) {
  global $mosConfig_lang;
  // shumisha end of fix
		$href= "index.php";
		$hrefVars = '';
		// V 1.2.4.m : we now use $_GET instead of $_SERVER['QUERY_STRING'] as this was causing problem when some parts of 
		// the URL was not in the query (ie params passed as &searchword=xxxxxxx for instance)
		if( !empty($_GET)) {
    ...
    // V 1.2.4.m removed as we now work on ly with sh404SEF
    //else {
    ...
    // temporarily switch language to get the proper translation
    $shCurrentLang = $mosConfig_lang;
		$mosConfig_lang = $shLangCode ? $shLangCode : $mosConfig_lang;
    $shSefUrl = sefRelToAbs ($href);
    $mosConfig_lang = $shCurrentLang;
    return $shSefUrl;
    // shumisha end of fix
	}
}
