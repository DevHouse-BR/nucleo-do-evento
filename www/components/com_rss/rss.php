...
function feedFrontpage( $showFeed ) {
	...
	$info[ 'title' ] 			= $params->def( 'title', 'Núcleo do Evento' );
	$info[ 'description' ] 		= $params->def( 'description', 'Notícias do site nucleodoevento.com.br' );
	$info[ 'image_file' ]		= $params->def( 'image_file', 'nucleodoevento_rss.png' );
	if ( $info[ 'image_file' ] == -1 ) {
		$info[ 'image' ]		= NULL;
	} else{
		$info[ 'image' ]		= $mosConfig_live_site .'/images/M_images/'. $info[ 'image_file' ];
	}
	$info[ 'image_alt' ] 		= $params->def( 'image_alt', 'Nucleo do Evento!' );
