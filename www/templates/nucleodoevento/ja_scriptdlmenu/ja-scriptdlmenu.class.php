...
class scriptdlmenu{
	...
	function genMenuItemBegin( &$row, $level, $pos ) {
		...
		$txt = "<span>$txt</span>";
		$className = ($pos == 0) ? "class=\"first-item\"" : "";
		...
		switch ($row->browserNav) {
			...
			$txt = "<a href=\"". $row->link ."\" target=\"_blank\" $className $title>". $txt . '</a>' ;
			break;

			case 2:
			// open in a popup window
			$txt = "<a href=\"#\" $className onclick=\"javascript: window.open('". $row->link ."', '', 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=780,height=550'); return false\" $title>". $txt . '</a>' ;
			break;

			case 3:
			// don't link it
			$txt = "<a href=\"#\" $className> $title". $txt . "</a>" ;
			break;

			default:	// formerly case 2
			// open in parent window
			$txt = "<a href=\"". $row->link ."\" $className $title>". $txt . "</a>" ;
			break;
		}
		...
		if ($level == 0) {
			$this->main .= "<li id=\"jasdl-mainnav{$row->id}\"".((@$this->parent->children[$row->id]) ? " class=\"havechild\"" : "").">$txt</li>\n";
			if (@$this->parent->children[$row->id]) {
				$this->sub .= "<ul id=\"jasdl-subnav{$row->id}\">\n";
				//$this->sub .= "<li class=\"spacer\">&nbsp;</li>";
			}
		} else if ($level == 1) {
			$this->sub .= "<li id=\"jasdl-subnavitem{$row->id}\">$txt</li>\n";
		}
	}
